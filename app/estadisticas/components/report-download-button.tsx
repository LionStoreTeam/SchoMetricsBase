...
export const ReportDownloadButton = () => {
    ...
    const generatePDFReport = async (data: UserReportData) => {
        ...
        // Título del reporte
        doc.setFontSize(22)
        doc.text("Reporte de Trayectoria - SchoMetrics", 105, y, { align: "center" })
        y += 15
        ...
        autoTable(doc, {
            startY: y,
            head: [["Fecha de Canje", "Recompensa", "Descripción", "Costo en Puntos"]],
        ...
        try {
            ...
            doc.setFontSize(10)
            doc.text("Este documento ha sido generado automáticamente por SchoMetrics.", 14, y)
            y += 5
            doc.text(
            ...
            doc.text(`Queda estrictamente prohibido la modificación parcial o total de esté documento.`, 14, y)
            y += 5
            doc.text(`Así como de la información existente en el mismo, con el fin de obtener beneficios adicionales.`, 14, y)
            y += 5
            doc.text(`En caso de realizar un incumplimiento se tomarán las medidas correspondientes establecidas en el Reglamento Escolar`, 14, y)
            y += 5
            ...
            doc.text("Para verificar la autenticidad de este documento, visite:", 14, y)
            y += 4
            doc.text(validationUrl, 14, y)
            y += 8
            doc.text(`© ${new Date().getFullYear()} SchoMetrics. Todos los derechos reservados.`, 14, y)
        } catch (error) {
        ...
        doc.save(`reporte_schometrics_${data.user.name?.replace(/\s/g, "_") || "usuario"}.pdf`)
    }

    const handleDownloadReport = async () => {
        setIsGenerating(true)
        try {
            toast.loading("Estamos recopilando tu información. Esto puede tardar un momento.")
            ...
            toast.error("Hubo un problema al generar tu reporte. Por favor, inténtalo de nuevo.")
        } finally {
    ...
    return (
        <Button onClick={handleDownloadReport} disabled={isGenerating} className="bg-teal-600 hover:bg-teal-700">
            {isGenerating ? (
                <>
