...
import { Loader2, Recycle, TreePine, Droplets, Zap, HousePlusIcon } from "lucide-react"
...
const generatePDFReport = (stats: GlobalStatsMetricsData, period: string) => {
    ...
    const primaryColor: [number, number, number] = [52, 211, 153] // Emerald-400
    ...
    doc.setTextColor(255, 255, 255)
    doc.setFontSize(10)
    doc.text(`Generado el: ${currentDate}`, pageWidth - 20, 15, { align: "right" })
    ...
    doc.setTextColor(255, 255, 255)
    doc.setFontSize(8)
    doc.text("SchoMetrics - Transformación educativa hacia la sostenibilidad.", pageWidth / 2, pageHeight - 10, {
        align: "center",
    })
...
const GlobalMetricsSchool = () => {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                setIsLoading(true)
                const response = await fetch(`/api/stats/global-metrics-school?period=${period}`)
                if (!response.ok) {
        ...
        return (
            <div className="flex flex-col justify-center items-center h-64 space-y-4">
                <div className="relative">
                    ...
                    <div className="absolute inset-0 h-16 w-16 animate-pulse bg-emerald-100 rounded-full blur-xl opacity-60"></div>
                </div>
                <p className="text-gray-600 font-medium animate-pulse">Cargando Métricas de Impacto...</p>
            </div>
        )
    }
    ...
    const metrics = [
        {
            ...
            percentage: Math.min((stats.impactMetrics.recycledMaterials / 1000) * 100, 100),
        },
        {
            icon: TreePine,
    ...
    return (
        <div className="mt-10 space-y-8">
            {/* Header mejorado */}
            <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-emerald-600 via-teal-600 to-teal-500 p-8 text-white">
                ...
                <div className="relative flex flex-col items-center justify-center lg:flex-row lg:items-start lg:justify-between">
                    <div className="space-y-2">
                        <div className="flex flex-col justify-center items-center text-center gap-3 lg:flex-row lg:items-start lg:text-left">
                            <div className="flex items-center justify-center w-12 h-12 bg-white/20 backdrop-blur-sm rounded-full">
                                <HousePlusIcon className="w-6 h-6 text-white" />
                            </div>
                            <div>
                                <h1 className="text-3xl font-bold">Impacto Ambiental</h1>
                                <p className="text-emerald-100">Midiendo el impacto sostenible de nuestra institución</p>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 mt-5" title="Descargar Reporte de Impacto Ambiental">
                        <Button
                            onClick={() => stats && generatePDFReport(stats, period)}
                            disabled={!stats}
                            className="bg-white/20 hover:bg-white/30 text-white border-white/30 hover:border-white/50 backdrop-blur-sm transition-all duration-300 group"
                        >
            ...
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
                {metrics.map((metric, index) => {
                    ...
                    return (
                        <Card
                            key={index}
                            className={`group relative overflow-hidden border-0 bg-gradient-to-br ${metric.bgGradient} hover:shadow-xl transition-all duration-500 hover:scale-105`}
                        >
                            <div className="absolute inset-0 bg-gradient-to-br from-white/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                            <CardContent className="relative p-6">
                                <div className="flex items-start justify-between mb-4">
                                    <div
                                        className={`flex items-center justify-center w-14 h-14 bg-gradient-to-br ${metric.gradient} rounded-2xl shadow-lg`}
                                    >
                                        <Icon className="w-7 h-7 text-white" />
                                    </div>
                                    <div className="opacity-60 group-hover:opacity-100 transition-opacity">
                                        <CircularProgress percentage={metric.percentage} size={60} strokeWidth={4} />
                                    </div>
                                </div>

                                <div className="space-y-2">
                                    <h3 className="font-bold text-teal-700 text-sm uppercase tracking-wide">{metric.title}</h3>
                                    <div className="flex items-baseline gap-1">
                                        <span className="text-3xl font-bold text-teal-500">
                                            <AnimatedCounter value={metric.value} decimals={metric.decimals} duration={1500 + index * 200} />
                                        </span>
                                        {metric.unit && <span className="text-lg font-medium text-teal-500">{metric.unit}</span>}
                                    </div>
                                </div>

                                {/* Detalles expandibles */}
                                <div className="mt-4 space-y-1 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                                    <div className="text-xs font-medium text-gray-700 mb-2">Equivalente a:</div>
                                    {metric.details.map((detail, detailIndex) => (
                                        <div key={detailIndex} className="flex items-center gap-2 text-xs text-gray-600">
                                            <div className={`w-1.5 h-1.5 bg-gradient-to-r ${metric.gradient} rounded-full`}></div>
                                            <span>{detail}</span>
                                        </div>
                                    ))}
                                </div>
