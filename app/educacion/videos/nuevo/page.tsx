...
function useUserSession() { /* ... (Hook de sesión existente) ... */
    const [session, setSession] = useState<{ id: string; userType: UserType; role: string, name: string, matricula: string } | null>(null);
...
export default function NewShortVideoPage() {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (!validationResult.success) {
            ...
            setErrors(newErrors); toast.error("Por favor, corrige los errores.");
        ...
        try {
            ...
            toast.success("Video corto creado!"); router.push("/educacion/videos"); router.refresh();
                                ...
                                <div className="space-y-1">
                                    ...
                                    <div className={`p-3 border-2 ${errors.videoFile ? 'border-red-500' : 'border-gray-300'} border-dashed rounded-lg`}>
                                        <div className="flex items-center justify-center w-full py-2 bg-gray-50 hover:bg-gray-100 rounded-md cursor-pointer" onClick={() => videoInputRef.current?.click()}>
                                            <input type="file" id="videoFile" name="videoFile" ref={videoInputRef} onChange={handleVideoFileChange} className="hidden" accept={ALLOWED_VIDEO_TYPES.filter(t => t !== "image/gif").join(",")} disabled={isSubmitting} />
