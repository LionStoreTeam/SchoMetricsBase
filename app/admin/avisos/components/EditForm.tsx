// app/ui/announcements/EditForm.tsx
"use client";
...
import { AlertCircle, ArrowLeft, Loader2, Megaphone, Save, Trash2 } from 'lucide-react';
...
export function EditAnnouncementForm({ announcement }: { announcement: Announcement }) {
    ...
    const handleUpdate = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const errorData = await response.json();
                setError(errorData.message || 'Error al actualizar. Verifica los campos.');
    ...
    if (isLoadingSession || isLoading) {
        return (
            <div className="min-h-screen flex justify-center items-center text-lime-400">
    ...
    const handleDelete = async () => {
        ...
        if (announcement.userId !== session.id || (session.userType !== UserType.ADMIN)) {
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <CardFooter className="flex flex-col justify-center gap-3 sm:flex-row sm:items-start sm:justify-end">
                            ...
                            <Button
                                type="submit"
                                disabled={isLoading}
                                className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-lime-600 hover:bg-lime-700 disabled:bg-gray-400">
