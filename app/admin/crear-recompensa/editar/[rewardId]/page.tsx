...
import {
    Gift, CalendarIcon, Loader2, Check,
} from "lucide-react";
...
export default function EditarRecompensa() {
    ...
    if (isLoading) {
        return (
            <div className="min-h-screen flex justify-center items-center text-amber-400 gap-4">
    ...
    return (
        <div className="flex flex-col gap-8 m-5 sm:m-10">
            <div className="p-6 flex flex-col gap-2 justify-center items-center lg:flex-row lg:justify-between text-white bg-gradient-to-r from-amber-500 to-amber-600 rounded-xl shadow-lg">
                <div>
                    <div className="flex items-center gap-3">
                        <Gift className="h-8 w-8" />
                        <h1 className="text-3xl font-bold tracking-tight">Eliminar o Editar Recompensa</h1>
                    </div>
                    <p className="text-amber-50">Elimina o Modifica los datos de la recompensa.</p>
                </div>
            </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <CardContent className="space-y-4">
                        <div>
                            ...
                            {errors.title && <p className="text-sm text-red-500">Campo requerido</p>}
                        </div>

                        <div>
                            <Label>Descripción</Label>
                            <Textarea rows={3} {...register("description", { required: true })} />
                            {errors.description && <p className="text-sm text-red-500">Campo requerido</p>}
                        </div>
