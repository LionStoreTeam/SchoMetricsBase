                            ...
                            <div className="overflow-x-auto">
                                <Table>
                                    <TableHeader><TableRow>
                                        ...
                                        <TableHead className="text-center">Tipo Usuario</TableHead>
                                            ...
                                            <TableRow key={activity.id} className={`hover:bg-gray-50 ${activity.status === ActivityStatus.PENDING_REVIEW ? 'bg-amber-50' : ''}`}>
                                                <TableCell>
                                                    <Avatar className="h-16 w-16">
                                                        ...
                                                        <AvatarFallback className="bg-indigo-100 text-indigo-700 text-xs font-semibold">{getInitials(activity.user.name)}
                                                        </AvatarFallback>
                                                    </Avatar>
                                                </TableCell>
                                                <TableCell className="font-medium text-sm text-center uppercase">{activity.user.name}
                                                </TableCell>
                                                <TableCell className="text-center text-[#2e86c1] font-bold text-lg">{activity.user.matricula}</TableCell>
                                                <TableCell className="text-center">
                                                    <Badge
                                                        variant={
                                                            activity.user.userType === "STUDENT" ? "secondary" :
                                                                activity.user.userType === "TEACHER" ? "outline" :
                                                                    activity.user.userType === "ADMIN" ? "outline"  // Example: use outline for community
                                                                        : "default"
                                                        }
                                                        className={
                                                            activity.user.userType === "STUDENT" ? "bg-sky-100 text-sky-700 border-sky-300" :
                                                                activity.user.userType === "TEACHER" ? "bg-green-100 text-green-700 border-green-300" :
                                                                    activity.user.userType === "ADMIN" ? "bg-red-950 text-white"
                                                                        : "bg-gray-100 text-gray-700 border-gray-300"
                                                        }
                                                    >
                                                        {USER_TYPE_MAP[activity.user.userType] || activity.user.userType}
                                                    </Badge>
                                                </TableCell>
                                                <TableCell className="text-sm text-center">{activity.title}</TableCell>
                                                <TableCell className="text-center">
                                                    <Badge className="text-xs whitespace-nowrap bg-sky-800 text-white">{ACTIVITY_TYPE_MAP[activity.type] || activity.type}</Badge>
                                                </TableCell>
                                                <TableCell className="text-center">
                                                    {activity.status === ActivityStatus.PENDING_REVIEW ? (
                                                        <Badge variant="destructive" className="text-xs whitespace-nowrap bg-amber-500 hover:bg-amber-600">
                                                            <Clock className="mr-1 h-3 w-3" />Por Calificar</Badge>
                                                    ) : (
                                                        <Badge variant="default" className="text-xs whitespace-nowrap bg-green-600 hover:bg-green-700">
                                                            <CheckCircle className="mr-1 h-3 w-3" />Calificada</Badge>
                                                    )}
                                                </TableCell>
                                                <TableCell className="text-lg text-center font-extrabold text-green-500">{activity.points}</TableCell>
                                                <TableCell className="text-center text-xs text-muted-foreground">{format(new Date(activity.createdAt), "dd MMM, yy", { locale: es })}</TableCell>
                                                ...
                                                <TableCell className="text-center flex justify-center items-center">
                                                    {activity.evidence && activity.evidence.length > 0 ? (
                                                        <EvidenceThumbnailsAdmin evidence={activity.evidence} />
