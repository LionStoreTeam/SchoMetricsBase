...
export const LinkPreview = ({
    ...
    const handleMouseMove = (event: React.MouseEvent<HTMLAnchorElement, MouseEvent>) => {
        const targetRect = (event.currentTarget as HTMLElement).getBoundingClientRect(); // Usar currentTarget
                    ...
                    <AnimatePresence>
                        {isOpen && (
                            ...
                            >
                                <a
                                    href={url}
                                    className="block p-1 bg-white border-2 border-transparent shadow rounded-xl hover:border-green-200"
                                    style={{ fontSize: 0 }}
